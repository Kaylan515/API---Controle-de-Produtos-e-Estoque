...
def inserir_produto(nome, categoria, preco, quantidade):
    ...
    if conexao:
        try:
            ...
            print(f"Erro ao inserir produto/item {erro}")
        finally:
            cursor.close()
            conexao.close()
...
def atualizar_produto(id_item, novo_preco, nova_quantidade):
    conexao, cursor = conectar()
    if conexao:
        try:
            cursor.execute(
                "UPDATE produtos SET preco = %s, quantidade = %s WHERE id = %s", 
                (novo_preco,nova_quantidade, id_item)
            )
            conexao.commit()
        except Exception as erro:
